...
public class ThumbnailCreator
{
    ...
    public byte[] SkiaCreate(Stream source, ImageSize desiredSize)
    {
        using (var fromBitmap = SkiaSharp.SKBitmap.Decode(source))
        {
            ...
            var newSize = resizer.Resize(originalSize, desiredSize);
            using (var toBitmap = new SKBitmap(newSize.Width, newSize.Height))
            {
                using (var canvas = new SKCanvas(toBitmap))
                {
                    canvas.DrawBitmap(fromBitmap, 0, 0);
                    canvas.Flush();
                    var image = SKImage.FromBitmap(toBitmap).ToRasterImage();
